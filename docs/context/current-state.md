...
> Last reviewed: 2026-07-14 (ProSE v0.3.0 — Keyword column, expiry
> strikethrough + reorder, user-formatting preservation)
...
| --- | --- | --- |
| ProSE — Professional Services Extractor | v0.3.0 | Scans Hawaii procurement sources (HANDS + HiePRO) for active solicitations matching keywords, records them into a styled Excel sheet with contact details, and emails it on a schedule. Local web dashboard (port 5000). A **Keyword** column records which keyword(s) each row matched (accumulated when a solicitation surfaces under several). **Expired** solicitations (past Due Date, local time) are struck through, greyed, and sorted to the bottom — recomputed every scan. The rewrite now **preserves the user's own cell borders + column widths** (detects the dominant applied border, only seeds default widths for undimensioned columns) instead of forcing program defaults. |
