...
Use `docs/highlights/` for public, sanitized, outward-facing career artifacts
derived from the project's history: engineering case studies (STAR-style),
a resume-bullet bank, and a skills matrix.

This is the mirror image of `docs/sessions/`. Sessions are private, internal,
and written for the next contributor. Highlights are public, curated, and
written for a recruiter or interviewer. They must contain no real client, firm,
personnel, or contact data, matching the repository's privacy posture.

Highlights are curated, not generated by a session-logging tool. Promote a
problem into a case study only when the author can explain it unaided, write in
an author-owned voice (specification, judgment, evaluation, verification), and
mark every metric as measured or estimated. Regenerate the bullets and matrix
from the case studies when they change.
